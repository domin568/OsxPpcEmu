...
TEST_F( MwpefldE2E, LinksStressTestObjectFile )
{
    const fs::path mwpefld{ m_sandbox / "cw" / "tools" / "mwpefld" };
    const fs::path libs{ m_sandbox / "cw" / "libs" };

    const std::vector<std::string> argv{
        EMU_BINARY,        mwpefld.string(), "-v", "-v", "-v", "src/cw_stress_test.cpp.o", "-o",
        "output/cw_stress_test",
    };
    ...
    const fs::path outputFile{ m_sandbox / "output" / "cw_stress_test" };
    ASSERT_TRUE( fs::exists( outputFile ) ) << "Linker did not produce an output file.\nstdout:\n"
                                            << result.stdoutText << "\nstderr:\n"
                                            << result.stderrText;
    EXPECT_GT( fs::file_size( outputFile ), 0u );

    const fs::path expected{ fixture_root() / "expected" / "cw_stress_test" };
