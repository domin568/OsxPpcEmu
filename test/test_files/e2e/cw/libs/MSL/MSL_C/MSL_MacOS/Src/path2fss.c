...
#else
static OSErr __path2fss_old(const char * pathName, FSSpecPtr spec)
...
OSErr __msl_path2splitfsr(const char * pathName, FSRefPtr theParentRef, HFSUniStr255 * theNewName)
{
	...
	while ((theErr == noErr) && separatorFound)
	{
		...
		if ((nextPosition - lastPosition) > 255)
			theErr = fnfErr;
		else
		{
			...
			if (separatorFound)
			{
				...
				else
				{
					...
					else
					{
						...
						if ((theErr == noErr) && ((UInt32) FSResolveAliasFileWithMountFlags != 0))
							theErr = FSResolveAliasFileWithMountFlags(theParentRef, true,
								&isFolder, &wasAlias, kResolveAliasFileNoUI);
					}
				}
			}
		}
...
OSErr __wpath2fss(const wchar_t * pathName, FSSpecPtr spec)
{
	OSErr theErr;
	FSRef theRef;
	FSCatalogInfo theInfo;
	...
	if (!pathName || !*pathName || wcslen(pathName) > 255)
		return (bdNamErr);
	
	theErr = __msl_wpath2fsr(pathName, &theRef);
	
	if (theErr == noErr)
		theErr = FSGetCatalogInfo(&theRef, kFSCatInfoNone, NULL, NULL, spec, NULL);
	else if (theErr == fnfErr)
	{
		theErr = __msl_wpath2splitfsr(pathName, &theRef, &theName);
		...
		if (theErr == noErr)
		{
			...
			if (theErr == noErr)
			{
				theName.length = wcslen(pathName);
				for (i = 0; i < theName.length; i++)
					theName.unicode[i] = pathName[i];
				__msl_unicode2text(&theName, &length, (char *) (&(thePName[1])));
				thePName[0] = length;
...
/* Turn a wchar_t pathname into a FSRef */
OSErr __msl_wpath2fsr(const wchar_t * pathName, FSRefPtr theRef)
{
	OSErr			theErr;
	Boolean			isFolder;
	Boolean			wasAlias;
	...
	theErr = __msl_wpath2splitfsr(pathName, &theParentRef, &theName);
...
/* Turn a wchar_t pathname into a parent FSRef and a filename */
OSErr __msl_wpath2splitfsr(const wchar_t * pathName, FSRefPtr theParentRef, HFSUniStr255 * theNewName)
{
	int i;
	int position;
	int lastPosition;
	...
	pathNameLength = wcslen(pathName);
	...
	if (pathName[0] == L':')
	{
		...
		while ((theErr == noErr) && (pathName[lastPosition] == L':'))
		{
			theRef = *theParentRef;
			theErr = FSGetCatalogInfo(&theRef, kFSCatInfoNone, NULL, NULL, NULL, theParentRef);
			lastPosition++;
	...
	else
	{
		...
		if (separatorFound)
		{
			theSpec.vRefNum = 0;
			theSpec.parID = 0;
			theNewName->length = position + 1;
			for (i = 0; i < position; i++)
				theNewName->unicode[i] = pathName[i];
			__msl_unicode2text(theNewName, &length, (char *) (&(theSpec.name[1])));
			theSpec.name[0] = length;
			theErr = FSpMakeFSRef(&theSpec, theParentRef);
		...
		else
		{
			...
			theNewName->length = pathNameLength;
	...
	while ((theErr == noErr) && separatorFound)
	{
		...
		theNewName->length = nextPosition - lastPosition;
		for (i = 0; i < nextPosition - lastPosition; i++)
			theNewName->unicode[i] = pathName[lastPosition + i];
 ...
 * JWW 030319 Fixed bug where filenames larger than 255 characters would overrun the filename buffer
 */