...
void SIOUXUpdateMenuItems(void)
{
	...
	if (SIOUXState != PRINTFING && SIOUXIsAppWindow(SIOUXFrontWindow())) {
	#if TARGET_API_MAC_CARBON
...
short SIOUXDoSaveText(void)
{
	short error;
	HParamBlockRec pb;
	StandardFileReply sfReply;	/*- ra 990612 -*/
...
RenameFile:
	...
	#else
		StandardPutFile("\pSave document as...", theTitle, &sfReply);	/*- ra 990612 -*/
		if (!sfReply.sfGood)
			goto NotSaved;
	#endif /* TARGET_API_MAC_CARBON */
	...
	#else
		SIOUXTextWindow->dirid = LMGetCurDirStore();	
	#endif /* TARGET_API_MAC_CARBON */
	...
	/*	Set the finder info ...*/
		...
		pb.fileParam.ioFlFndrInfo.fdType = __gettype(false);
		pb.fileParam.ioFlFndrInfo.fdCreator = __getcreator(false);
#else
		pb.fileParam.ioFlFndrInfo.fdType = 0;
		pb.fileParam.ioFlFndrInfo.fdCreator = 0;
#endif
...
void SIOUXDoMenuChoice(long menuValue)
{
	...
	if (!SIOUXIsAppWindow(SIOUXFrontWindow()))
		return;
...
/* Change record:
 * BB  931001 removed all TextEdit calls which called toolbox glue
 *			  so that users no longer needed to include MacOS.lib ...
 * BB  931004 rewrote savetext so that all file saving was done using
 *			  inline toolbox calls therefore no MacOS.lib ...
 * JH  951230 removed uses of OLDROUTINENAMES
 * bk  960902 added Universal Headers incase macheaders not the prefix
 * mm  991108 added ra Carbon Changes done 990611
 ...
 * JWW 010122 Don't call AppendResMenu() to populate the Apple menu when targeting Carbon
 * JWW 010917 Added floating window mode to SIOUX
 * JWW 010927 Use __gettype and __getcreator accessors instead of directly getting the values
