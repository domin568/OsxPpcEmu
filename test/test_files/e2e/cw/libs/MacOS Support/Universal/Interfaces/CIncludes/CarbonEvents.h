/*
     ...
     Version:    Technology: CarbonLib 1.6
 ...
 *    the mouse is moved. On Mac OS X after 10.1 and CarbonLib after
 *    1.5, TrackMouseLocation, TrackMouseLocationWithOptions, and
...
enum {
   ...
   * this flag will still be set. Available in Mac OS X after 10.1, and
   * in CarbonLib after 1.5.
   */
...
enum {
   ...
   * Mac OS X after 10.1, and CarbonLib after 1.5, this event is also
   * sent by ResizeWindow and GrowWindow if the sizeConstraints
   * parameter is NULL.
   */
   ...
   * change. In Mac OS X after 10.1, this event may optionally contain
   * a kEventParamDockChangedReason parameter; if present, this
   * parameter is a constant indicating the reason why the Dock changed
   * size and caused the constrain event to be sent. The standard
   * window handler will not move windows if the DockChangedReason is
   * kDockChangedUser. In Mac OS X after 10.1, this event may
   * optionally contain kEventParamPreviousDockRect and
   * kEventParamCurrentDockRect parameters; if present, these
   * parameters contain the previous and current bounds of the Dock.
   */
...
/*
 *  Summary:
 *    Reasons why the Dock rect changed.
 *  
 *  Discussion:
 *    The kEventParamDockChangedReason parameter to the
 *    kEventWindowConstrain event contains one of these constants.
 */
enum {

  /*
   * The rect changed because the user resized the dock somehow.
   */
  kDockChangedUser              = 1,

  /*
   * The rect changed because the user changed the orientation of the
   * dock.
   */
  kDockChangedOrientation       = 2,

  /*
   * The rect changed because autohide was turned on or off.
   */
  kDockChangedAutohide          = 3,

  /*
   * The rect changed due to a screen resolution change.
   */
  kDockChangedDisplay           = 4,

  /*
   * The rect changed because items were added/removed from the dock.
   */
  kDockChangedItems             = 5,

  /*
   * The rect changed for some other reason.
   */
...
/*
    ...
    kEventWindowConstrain
        ...
        --> kEventParamAttributes       typeUInt32 (optional; available in Mac OS X after 10.1)
        --> kEventParamWindowRegionCode typeWindowRegionCode (optional; available in Mac OS X after 10.1)
        --> kEventParamRgnHandle        typeQDRgnHandle (optional; available in Mac OS X after 10.1)
        --> kEventParamDockChangedReason    typeUInt32 (available in Mac OS X after 10.1)
        --> kEventParamPreviousDockRect     typeHIRect (available in Mac OS X after 10.1)
        --> kEventParamCurrentDockRect      typeHIRect (available in Mac OS X after 10.1)
...
enum {
   ...
   * 
   * In Mac OS X after 10.1, kEventMenuPopulate is also sent to menus
   * before the menu is searched for a command ID by the
...
enum {
   ...
   * Available on Mac OS X after 10.1.
   */
...
/*
    ...
    kEventMenuPopulate (on Mac OS X 10.1 and later, and CarbonLib 1.5 and later)
        -->     kEventParamDirectObject             typeMenuRef
        -->     kEventParamMenuContext              typeUInt32
        -->     kEventParamMenuCommand              typeMenuCommand (Mac OS X after 10.1, for kMenuContextCommandIDSearch)
...
enum {
   ...
   * will be included on Mac OS X after 10.1, indicating whether the
   * command was sent from a menu in the menubar (kMenuContextMenuBar
   * will be set) or from a popup menu (kMenuContextMenuBar will not be
   * set). The MenuContext parameter also indicates whether the event
   * was sent by a selection of a menu item with the mouse
   * (kMenuContextMenuBarTracking or kMenuContextPopUpTracking will be
   * set), or by a command key press (kMenuContextKeyMatching will be
   * set). 
   * 
...
/*
    ...
    kEventCommandProcess
        ...
        -->     kEventParamMenuContext      typeUInt32 (optional, in Mac OS X after 10.1)
...
enum {
  ...
  kEventParamDockChangedReason  = FOUR_CHAR_CODE('dcrs'), /* typeUInt32*/
