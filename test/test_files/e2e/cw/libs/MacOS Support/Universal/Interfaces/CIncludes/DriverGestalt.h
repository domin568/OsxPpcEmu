...
enum {
  ...
  kdgGetDriveAddInfo            = FOUR_CHAR_CODE('digt'), /* Get a disk driver's add-drive information record */
  kdcAddDriveWithInfo           = FOUR_CHAR_CODE('diad'), /* Tell disk driver to add the drive specified with the drive information record */
...
enum {
  ...
  driveInfoInteractMask         = (long)0x80000000, /*  to perform user interaction to add the drive */
  driveInfoChangedBit           = 30,   /* Output from kdgGetDriveInfo: If set, the drive was mounted, but */
  driveInfoChangedMask          = 0x40000000, /*  the drive information record needs to be updated. */
  driveInfoDriverReservedMask   = 0x0000FFFF, /* bits 0-15 are defined by each driver for its own use */
  driveInfoSystemReservedMask   = (long)0xFFFF0000 /* bits 16-31 are reserved for Apple system use */
};
...
struct CDDeviceCharacteristics {
  ...
  UInt16              extendedCdFeatures;     /* extended flags to support new DVD-ROM/DVD-R etc. */
};
