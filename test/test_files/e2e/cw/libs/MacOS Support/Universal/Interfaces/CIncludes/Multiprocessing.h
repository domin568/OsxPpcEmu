...
#define MPCopyrightNotice   \
    "Copyright © 1995-2001 Apple Computer, Inc.\n"
...
enum {
  ...
  kMPCreateTaskValidOptionsMask = kMPCreateTaskTakesAllExceptionsMask | kMPCreateTaskNotDebuggableMask
};
...
MPWaitForEvent(
  MPEventID       event,
  MPEventFlags *  flags,         /* can be NULL */
  Duration        timeout);
...
MPCancelTimer(
  MPTimerID       timerID,
  AbsoluteTime *  timeRemaining);      /* can be NULL */
...
EXTERN_API_C( void )
_MPLibraryVersion(
  const char **  versionCString,       /* can be NULL */
